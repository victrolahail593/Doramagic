...
class TestStage0:
    ...
    def test_stage0_success(self, tmp_repo, tmp_output):
        ...
        with (
            patch("phase_runner._find_extract_script", return_value="/fake/extract_repo_facts.py"),
            patch("subprocess.run", return_value=mock_result) as mock_run,
            patch("phase_runner._import_extraction_modules", return_value=(None, None, None, None)),
            patch("phase_runner._import_validate", return_value=(None, None)),
        ):
    ...
    def test_stage0_script_not_found(self, tmp_repo, tmp_output):
        ...
        with (
            patch("phase_runner._find_extract_script", return_value=None),
            patch("phase_runner._import_extraction_modules", return_value=(None, None, None, None)),
            patch("phase_runner._import_validate", return_value=(None, None)),
        ):
            result = run_single_project_pipeline(
    ...
    def test_stage0_subprocess_failure(self, tmp_repo, tmp_output):
        ...
        with (
            patch("phase_runner._find_extract_script", return_value="/fake/script.py"),
...
class TestBrickInjection:
    ...
    def test_bricks_loaded_successfully(self, tmp_repo, output_with_facts, tmp_path):
        ...
        with (
            ...
            patch("phase_runner._import_extraction_modules", return_value=(None, None, None, None)),
            patch("phase_runner._import_validate", return_value=(None, None)),
            patch("phase_runner._import_brick_injector", return_value=mock_injector),
        ):
            result = run_single_project_pipeline(
                repo_path=str(tmp_repo),
                output_dir=str(output_with_facts),
...
class TestStage15:
    ...
    def test_stage15_success(self, tmp_repo, output_with_facts):
        ...
        with (
            patch("phase_runner._find_extract_script", return_value=None),
            patch(
                "phase_runner._import_extraction_modules",
...
class TestStage35:
    ...
    def test_validation_pass_allows_stage45(self, tmp_repo, output_with_cards):
        ...
        with (
            ...
            patch("phase_runner._import_validate", return_value=(mock_validate_all, mock_write_report)),
        ):
            result = run_single_project_pipeline(
                repo_path=str(tmp_repo),
                output_dir=str(output_with_cards),
...
class TestStage5:
    ...
    def test_stage5_success(self, tmp_repo, output_with_soul_and_validation):
        ...
        with (
            ...
            patch("phase_runner._import_validate", return_value=(mock_validate_all, mock_write_report)),
            patch("phase_runner._import_assemble", return_value=fake_assemble),
        ):
            result = run_single_project_pipeline(
                repo_path=str(tmp_repo),
    ...
    def test_stage5_skipped_by_config(self, tmp_repo, output_with_cards):
        ...
        with (
            ...
            patch("phase_runner._import_validate", return_value=(mock_validate_all, mock_write_report)),
            patch("phase_runner._import_assemble", return_value=mock_assemble),
        ):
            result = run_single_project_pipeline(
                repo_path=str(tmp_repo),
                output_dir=str(output_with_cards),
...
class TestGracefulDegradation:
    ...
    def test_all_modules_missing_no_crash(self, tmp_repo, tmp_output):
        ...
        with (
            ...
            patch("phase_runner._import_extraction_modules", return_value=(None, None, None, None)),
            patch("phase_runner._import_validate", return_value=(None, None)),
            patch("phase_runner._import_assemble", return_value=None),
            patch("phase_runner._import_brick_injector", return_value=None),
        ):
            result = run_single_project_pipeline(
                repo_path=str(tmp_repo),
                output_dir=str(tmp_output),
