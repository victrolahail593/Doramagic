...
def _file_evidence(path: str, snippet: Optional[str] = None) -> EvidenceRef:
    return EvidenceRef(
        kind="file_line",
        path=path,
        snippet=snippet,
...
def _extract_q1(inp: Stage1ScanInput, repo_id: str, seq_start: int) -> list[Stage1Finding]:
    ...
    if rf.languages and rf.frameworks:
        ...
        findings.append(Stage1Finding(
            ...
            confidence="high",
            evidence_refs=[_artifact_evidence("repo_facts.json", "languages+frameworks+entrypoints")],
        ))
        seq += 1
    elif rf.languages:
        langs = ", ".join(rf.languages[:3])
...
def _extract_q2(inp: Stage1ScanInput, repo_id: str, seq_start: int) -> list[Stage1Finding]:
    ...
    _ai_sdk_deps = {"openai", "@openai", "anthropic", "@anthropic", "@vercel/ai", "langchain", "llmchain"}
...
def _extract_q3(inp: Stage1ScanInput, repo_id: str, seq_start: int) -> list[Stage1Finding]:
    ...
    if rf.frameworks:
        ...
        rationale = rationale_map.get(fw, (
    ...
    if is_nextjs and has_api_route:
        findings.append(Stage1Finding(
            ...
            evidence_refs=[
                ...
                for ep in rf.entrypoints if "route" in ep.lower()
            ][:2],
        ))
        seq += 1
...
def _generate_hypotheses(
    ...
    if has_ai and has_zod:
        hypotheses.append(Hypothesis(
            ...
            priority="high",
            search_hints=["z.object", "z.parse", "safeParse", "schema", "validate", "parseJSON"],
...
def _compute_coverage(findings: list[Stage1Finding]) -> Stage1Coverage:
    ...
    for q in all_questions:
        ...
        if not confs:
            ...
            answered.append(q)
        elif any(c == "medium" for c in confs):
            answered.append(q)
        else:
...
def run_stage1_scan(
    input: Stage1ScanInput,  # noqa: A002  (intentional mirror of contract param name)
    ...
    recommended = (
        ...
        and len(coverage.answered_questions) > 0
    )
