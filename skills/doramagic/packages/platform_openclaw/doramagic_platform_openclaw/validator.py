...
from doramagic_contracts.envelope import ErrorCodes, ModuleResultEnvelope, RunMetrics, WarningItem  # noqa: E402
...
DARK_TRAP_PATTERNS = [
    (re.compile(r"\bcron\b", re.IGNORECASE), "cron job reference found (OpenClaw does not support cron)"),
...
def _read_file(path_str: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    if not path.exists():
        return None, "File not found: {0}".format(path_str)
    try:
        content = path.read_text(encoding="utf-8")
        return content, None
    except Exception as exc:  # noqa: BLE001
        return None, "Cannot read {0}: {1}".format(path_str, exc)
...
def _blocked_envelope(error_code: str, wall_time_ms: int = 0) -> ModuleResultEnvelope[ValidationReport]:
...
def _check_consistency(skill_md: str, body: str) -> ValidationCheck:
    ...
    for heading in headings:
        ...
        if normalized in seen:
            details.append("Duplicate section heading: '{0}'".format(heading.strip()))
        seen.add(normalized)
    ...
    if clawd and non_clawd:
        details.append(
            "Mixed storage paths detected: non-~/clawd/ paths {0} alongside ~/clawd/ paths".format(non_clawd[:3])
        )
...
def _check_completeness(skill_md: str, need_profile_keywords: List[str]) -> ValidationCheck:
    ...
    if missing_keywords:
        details.append("Keywords from need_profile not found in skill: {0}".format(missing_keywords))

    # Check that at least one actionable capability keyword exists in the body
    if not CAPABILITY_KEYWORDS.search(skill_md):
        details.append("SKILL.md contains no actionable capability keywords (read/write/exec/store/parse etc.)")
...
def _check_platform_fit(frontmatter: str, platform_rules_allowed_tools: List[str],
                        platform_rules_whitelist: List[str], platform_rules_forbidden: List[str],
                        storage_prefix: str, skill_md: str) -> ValidationCheck:
    ...
    if not actual_tools:
        ...
        if invalid_tools:
            details.append(
                "Frontmatter 'allowed-tools' contains non-allowed tools: {0} (allowed: {1})".format(
                    invalid_tools, platform_rules_allowed_tools
                )
            )
    ...
    for forbidden_key in platform_rules_forbidden:
        if forbidden_key in fm_keys:
            details.append("Frontmatter contains forbidden field: {0}".format(forbidden_key))
    ...
    for key in fm_keys:
        ...
        if key not in whitelist_set and normalized_key not in {k.replace("-", "_") for k in whitelist_set}:
            details.append("Frontmatter key '{0}' is not in OpenClaw metadata whitelist".format(key))
    ...
    if bad_paths:
        details.append(
            "Storage paths not using required prefix '{0}': {1}".format(storage_prefix, bad_paths[:5])
        )
...
def _check_conflict_resolution(skill_md: str, limitations_md: str) -> ValidationCheck:
    ...
    for text, label in [(skill_md, "SKILL.md"), (limitations_md, "LIMITATIONS.md")]:
        ...
        if matches:
            details.append("{0} contains unresolved conflict markers: {1}".format(label, matches[:3]))
...
def _check_license(provenance_md: str) -> ValidationCheck:
    ...
    if not has_license:
        details.append("PROVENANCE.md contains no license information — license compatibility cannot be verified")
    else:
...
def _check_dark_trap_scan(skill_md: str, limitations_md: str, readme_md: str) -> ValidationCheck:
    ...
    for pattern, description in DARK_TRAP_PATTERNS:
        if pattern.search(combined):
            details.append("Dark trap detected: {0}".format(description))
        ...
        if not check.passed and check.severity == "blocking":
            for detail in check.details:
                if check.name == "Platform Fit" and "forbidden field" in detail:
                    ...
                    instructions.append(
                        "Remove '{0}' from SKILL.md frontmatter. "
                        "If scheduling is needed, document it in README installation notes.".format(field)
                    )
                ...
                elif check.name == "Platform Fit" and "not in OpenClaw metadata whitelist" in detail:
                    instructions.append(
                    ...
                    instructions.append(
                        "Fix {0} issue: {1}".format(check.name, detail)
                    )
    return instructions
...
def run_validation(input_data: ValidationInput) -> ModuleResultEnvelope[ValidationReport]:
    ...
    for wf in warning_failures:
        for detail in wf.details:
            envelope_warnings.append(WarningItem(code=wf.name.upper().replace(" ", "_"), message=detail))
