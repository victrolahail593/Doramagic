...
def _load_repo_code(repo_path: str, output_dir: str, max_chars: int = 100_000) -> str:
    ...
    common = ["README.md", "README.rst", "setup.py", "pyproject.toml",
              "package.json", "Cargo.toml", "go.mod", "pom.xml",
              "index.js", "main.py", "lib.rs", "main.go", "src/index.ts"]
...
def _find_stages_dir() -> Optional[Path]:
    ...
    candidates = [
        ...
        this_file.parent.parent.parent.parent.parent.parent / "skills" / "soul-extractor" / "stages",
    ]
    for c in candidates:
        if c and c.is_dir():
            return c
...
def _load_stage_instructions(stage_name: str) -> Optional[str]:
    """加载 STAGE-*.md 指令文件。"""
...
def _run_stage1(repo_path, output_dir, code_context, adapter, model_id) -> StageResult:
    ...
    try:
        ...
        if response.is_refusal:
            return StageResult(stage="stage1", success=False, model_id=model_id,
                             error="LLM refused", wall_time_ms=_elapsed(start))

        files = _write_soul_essence(output_dir, response.content)
        return StageResult(
            stage="stage1", success=True, model_id=model_id, output_files=files,
        ...
        return StageResult(stage="stage1", success=False, model_id=model_id,
                         error=str(e), wall_time_ms=_elapsed(start))
...
def _run_stage2(repo_path, output_dir, code_context, adapter, model_id) -> StageResult:
    ...
    try:
        ...
        if response.is_refusal:
            return StageResult(stage="stage2", success=False, model_id=model_id,
                             error="LLM refused", wall_time_ms=_elapsed(start))

        files = _parse_and_write_cards(output_dir, response.content, "concepts")
        return StageResult(
            stage="stage2", success=len(files) > 0, model_id=model_id,
            output_files=files, error="" if files else "No cards parsed from LLM output",
            prompt_tokens=response.prompt_tokens, completion_tokens=response.completion_tokens,
            wall_time_ms=_elapsed(start),
        )
    except Exception as e:
...
def _run_stage3(repo_path, output_dir, code_context, adapter, model_id) -> StageResult:
    ...
    community = community_path.read_text(encoding="utf-8") if community_path.exists() else "[No community signals available]"
    instructions = instructions.replace("<output>", output_dir)
    ...
    try:
        ...
        if response.is_refusal:
            return StageResult(stage="stage3", success=False, model_id=model_id,
                             error="LLM refused", wall_time_ms=_elapsed(start))

        files = _parse_and_write_cards(output_dir, response.content, "rules")
        return StageResult(
            stage="stage3", success=len(files) > 0, model_id=model_id,
            output_files=files, error="" if files else "No cards parsed from LLM output",
            prompt_tokens=response.prompt_tokens, completion_tokens=response.completion_tokens,
            wall_time_ms=_elapsed(start),
        )
...
def run_llm_stages(
    ...
    *,
    ...
    logger.info(
        ...
        result.stages_completed, result.stages_failed, result.stages_skipped,
